...
func AddResourceRevocable(resource *mesos.Resource, principal string, role string) *mesos.Resource {
        resource.Revocable = &mesos.Resource_RevocableInfo{}
        return resource
}

func NewScalarResourceWithRevocable(name string, value float64, principal string, role string) *mesos.Resource {
        return AddResourceRevocable(NewScalarResource(name, value), principal, role)
}
